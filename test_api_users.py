...
def test_register():
   ...
   response = client.post("/account/register", content=user_input.model_dump_json())
...
def test_register_existing_username():
    ...
    response_exising_username = client.post(f"/account/register",content=new_user_in.model_dump_json())
    assert response.status_code == 200
